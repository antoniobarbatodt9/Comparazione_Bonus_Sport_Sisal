...
//   node render_horizontal.mjs --size 1920x1080 --times 0.6,1.5 --out ../300x600/storyboard --prefix sb [--mode wire|final] [--safe 1] [--grid 1] [--scale 1]
//   node render_horizontal.mjs --size 300x250 --fps 25 --duration 10 --out <dir>     (sequenza completa: SOLO dopo approvazione)
...
const size = args.size || '1920x1080'; const [W, H] = size.split('x').map(Number); const out = resolve(args.out || '.'); mkdirSync(out, { recursive: true });
...
const base = pathToFileURL(resolve(here, 'banner_horizontal.html')).href; let i = 0; const boxes = [];
