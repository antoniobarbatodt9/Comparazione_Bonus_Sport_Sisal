...
## Concept M — montaggio a scene (PRODOTTO il 2026-09-10; REVISIONE 2 applicata: niente effetto vetro, ambiente stadio reale da clip Higgsfield con audio nativo, solo pulse sulla CTA finale)
Secondo concept richiesto dal cliente: scene montate come clip separate con transizioni (S1 "BONUS SPORT  SCOPRI DI PIÙ" a tutto banner, ingresso BONUS → SPORT → CTA → Sisal → NetBet → William Hill uno per volta con CTA media accanto alla card → comparazione → CTA in risalto), **12,0 s = 300 fotogrammi**, sfondo **campo di calcio visto dall'alto** (Higgsfield), CTA presente in ogni scena, **traccia audio: ambiente stadio reale estratto da una clip Higgsfield con audio nativo + effetti discreti sintetizzati (mai voiceover)**. Approvazioni del cliente: 12 s · sfondo zenitale · audio ok · nessun accento sulla card Sisal in S5 · indici "n/3" rimossi.
